...
public class ArticleReadApiTest {
    ...
    void readTest() {
        ArticleReadResponse response = articleReadRestClient.get()
                .uri("/v1/articles/{articleId}", 172878599449669632L)
                .retrieve()
    ...
    void readAllTest() {
        ...
        for (ArticleReadResponse article : response2.getArticles()) {
            System.out.println("response2.getArticleId() = " + article.getArticleId());
        }
    }
    ...
    void readAllInfiniteScrollTest() {
        List<ArticleReadResponse> response1 = articleReadRestClient.get()
                .uri("/v1/articles/infinite-scroll?boardId=%s&pageSize=%s&lastArticleId=%s".formatted(1L, 5L, 174843786726227968L))
                .retrieve()
